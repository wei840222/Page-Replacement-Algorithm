...
class RepALG {
 public:
  ...
  virtual bool isFinish() const {
    return !(_accessNumber < _accessSequence.size());
  }
  virtual bool isPageFault() const {
    return (_frameStatus.find(_accessSequence[_accessNumber]) == string::npos)
               ? true
               : false;
  }
 ...
 protected:
  RepALG(string s = "", int i = 1)
      : _accessSequence(s), _accessNumber(0), _frameStatus(""), _frameSize(i) {}
